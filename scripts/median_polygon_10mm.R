...
# ------------------------------------------------------------
# This is a SEPARATE, independent script from scripts/median_polygon_2deg.R,
# not a superset of it: median_polygon_2deg.R bins by a fixed ANGLE (2 deg)
...
#      is ~8 mm of arc on a 450 mm stem but ~33 mm on a 1900 mm giant -- 4x
#      the denoising power on the giant, 4x less on the small stem, for no
...
#      radius -- an approximation of the true (possibly fluted) surface arc
#      length, not an exact measurement of it, since the real surface isn't
#      defined until after this same binning step builds it.
...
OUTPUT_COL <- "Dendrometer_MedianPolygon10mm_RScript_Diameter_mm"
...
flag_names <- c("--tree-id", "--up-axis", "--height", "--thickness",
                "--bin-width-mm", "--min-coverage", "--out", "--poly-dir")
...
# Target arc-length bin width (mm of ring circumference), not a fixed degree
# width -- see header comment for why. Converted to an angular bin count
# per-tree below, using that ring's own mean radius.
bin_width_mm <- as.numeric(get_flag("--bin-width-mm", 10))
...
# ------------------------------------------------------------------ visualization
# If poly_dir is set, write <tree_id>_median_polygon_10mm_r.ply (cyan, the
# median-binned surface polygon) and <tree_id>_median_hull_10mm_r.ply
# (magenta, its convex hull) for loading in CloudCompare beside the original
# disc/slice. `_10mm_r` suffix keeps these from colliding with
# median_polygon_2deg.R's `_2deg` output and with the Python `_py` variant
# when both are pointed at the same folder.
...
measure_one <- function(path, tree_id, up_axis, height, thickness, bin_width_mm, min_cov, poly_dir) {
  ...
  ring_radius_mm <- 1000 * mean(radii)                          # representative radius (see header)
  n_bins    <- max(round(2 * pi * ring_radius_mm / bin_width_mm), 3)
  bin_w_rad <- 2 * pi / n_bins
  bin_width_deg_equiv <- bin_w_rad * 180 / pi
  ...
  if (!is.null(poly_dir)) {
    ...
    write_ply_xyzrgb(file.path(poly_dir, sprintf("%s_median_polygon_10mm_r.ply", tree_id)),
                     poly_closed_3d, c(0, 200, 200))
    write_ply_xyzrgb(file.path(poly_dir, sprintf("%s_median_hull_10mm_r.ply", tree_id)),
  ...
  data.frame(
    ...
    bin_width_deg_equiv             = round(bin_width_deg_equiv, 2),
    n_bins                          = n_bins,
...
if (from_sheet) {
  ...
  for (m in manifest) {
    ...
    result <- tryCatch(
      measure_one(m$path, m$tree_id, up_axis, h, thickness, bin_width_mm, min_cov, poly_dir),
  ...
  row <- measure_one(path, tree_id, up_axis, height, thickness, bin_width_mm, min_cov, poly_dir)
  report_row(row)
  all_rows <- row
}
